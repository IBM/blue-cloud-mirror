...
function classifyImage() {
	...
	var file = document.getElementById('classify-input').files[0];
	...
	var form = new FormData();
	form.append("images_file", file);

	xhrAttach(url, form, function(item){
		document.getElementById('classification').innerHTML = "found classification for \""+item.result.images[0].image+"\": <br>"
			+ item.result.images[0].classifiers[0].name + ": "+item.result.images[0].classifiers[0].classes[0].class + " ("+item.result.images[0].classifiers[0].classes[0].score+")";
		console.log(item);
	}, function(err){
		console.error(err);
	});
...
function previewFile(){
	var preview = document.getElementById('classify-image'); //selects the query named img
	var file    = document.getElementById('classify-input').files[0]; //sames as here
	var reader  = new FileReader();

	reader.onloadend = function () {
		preview.src = reader.result;

		document.getElementById('classify-button').innerHTML = "<button onclick='classifyImage()'>classify Image</button>";
		// preview.innerHTML += "<br>classify Image "+"<button onclick='toggleAppInfo()'></button>";
	}
