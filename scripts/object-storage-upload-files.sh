...
function setup() {
  ...
  cd ${root_folder}/
  npm install
  cp ${root_folder}/../game/vue.config.js.cos.template ${root_folder}/../game/vue.config.js

  npm --prefix ${root_folder}/text-replace start ${root_folder}/text-replace ${root_folder}/../game/vue.config.js replace-me ${BUCKET_NAME}/
  ...
  for f in * **/* **/**/*; do
    ...
    case $f in
      *.ico*)
        curl -X "PUT" "https://s3.us-south.objectstorage.softlayer.net/${BUCKET_NAME}/${f}" \
  ...
  printf "\nGAME_URL=https://s3.us-south.objectstorage.softlayer.net/$BUCKET_NAME/index.html" >> $ENV_FILE
...
source $ENV_FILE
export IBMCLOUD_ORG IBMCLOUD_API_KEY BLUEMIX_REGION APPID_TENANTID APPID_OAUTHURL APPID_CLIENTID APPID_SECRET CLOUDANT_USERNAME CLOUDANT_PASSWORD IBMCLOUD_SPACE COS_ID BUCKET_NAME
