...
function setup() {
  ...
  ibmcloud resource service-instance-create object-storage-emotions cloud-object-storage lite global
  
  _out Creating Object Storage credentials
  ibmcloud resource service-key-create object-storage-emotions-credentials Reader --instance-name object-storage-emotions
    
  COS_ID=$(ibmcloud resource service-instance object-storage-emotions | awk '/ID: /{ print $2 }')
  _out COS_ID: $COS_ID
  printf "\nCOS_ID=$COS_ID" >> $ENV_FILE

  _out Object Storage service object-storage-emotions has been created
}
